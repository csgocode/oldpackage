...
def Headers(method):
    ...
    if method == "get" or method == "head":
        ...
        useragent = "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) HobbaHotel/1.0.0 Chrome/85.0.4183.121 Electron/10.2.0 Safari/537.36\r\n"
        header = referer + useragent + accept + connection + "\r\n\r\n"
    elif method == "post":
...
def downloadsocks(choice):
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("http://77.68.116.208/http.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
